...
s3MouseEvent::s3MouseEvent(s3ButtonType type, int x, int y, int offsetX, int offsetY, float scroll, bool control, bool shift)
    : type(type), x(x), y(y), offsetX(offsetX), offsetY(offsetY), scroll(scroll), control(control), shift(shift)
{
}
...
s3KeyEvent::s3KeyEvent(s3KeyCode keyCode, unsigned int key, bool control, bool shift, bool alt)
    : keyCode(keyCode), key(key), control(control), shift(shift), alt(alt)
{
}
...
//        s3KeyStateToString((s3KeyState)state).c_str(), 
//        s3KeyTypeToString(keyCode).c_str(), (unsigned char) key, s3ToString(control).c_str(), s3ToString(shift).c_str(), s3ToString(alt).c_str());
//}
